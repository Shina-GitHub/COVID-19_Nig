source("solve_fact_July15.R") # for adjusting for all cases
# #libraries needed
library(tidyverse)
library(dplyr)
...
####Reff
##adjust for reporting of symptomatic cases\
loc_case1 = mydaily_local$cases  #p= 0.1
loc_case2 =  mydaily_local$cases  #p= 0.5
import_case1 = mydaily_import$cases #p= 0.1
import_case2 = mydaily_import$cases #p= 0.5
##############
for (it in 1:length(mydaily_local$cases)){
  #multiply by 2 for 50% asymptomatic
    loc_case1[it] = 2*local_adjusted(loc_case1[it], 0.1)
    loc_case2[it] = 2*local_adjusted(loc_case2[it], 0.5)
    import_case1[it] = 2*local_adjusted(import_case1[it], 0.1)
    import_case2[it] = 2*local_adjusted(import_case1[it], 0.5)
}
for (ip in 1:4){

if (ip == 1){loc_case = loc_case1
    import_case = import_case1
    all_count = loc_case + import_case}
else if (ip == 2){loc_case = loc_case1
import_case = round(40*import_case1/100)
all_count = loc_case + import_case
}
else if (ip == 3){loc_case = loc_case2
import_case = import_case2
all_count = loc_case + import_case
}
else if (ip == 4){loc_case = loc_case2
import_case = round(40*import_case2/100)
all_count = loc_case + import_case
}
  #Initialise R
...
# Nice Ploting
#
#data_plot1 = data.frame(days = covid_nig_data_all$my_date, import =import_inc, local_cases = local_inc)
...
if (ip == 1){p1=p_plot}
else if (ip==2){p2=p_plot}
else if (ip==3){p3=p_plot}
else if (ip==4){p4=p_plot}
}
