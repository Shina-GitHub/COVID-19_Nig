...
all_count = mydaily_local$cases + mydaily_import$cases
  ...
  #Save Reff
write.csv(Rt_est,"Reff.csv_May7.csv")
min(Rt_est)
max(Rt_est)
# 
...
# Nice Ploting
#
data_plot1 = data.frame(days = daily_date, import = mydaily_import$cases, local_cases = mydaily_local$cases)
...
p2<-ggplot(data= data_plot1,aes(x=days,y=import, fill = 'r1'))+
  ...
  labs(x = "Days", y=paste("SARS-CoV-2 incidence", paste0("(","Nigeria",")")))+
